    ...
    return (
      <Page>
        <Helmet
          ...
          meta={[
            {
              name: "description",
              content:
                "From supply chain and procurement to revenue management and marketing, some of the world’s largest organizations are achieving breakthrough agility by transforming how they make and execute decisions."
            },
            {
          ...
          link = {[
            { rel : "canonical", href : "https://www.aeratechnology.com/customers" }
          ]}
        />

        
        <Intro title={heading} text={subheading} fullheight/>

        <Content>
          {allCustomers &&
            allCustomers
              .slice(0, allCustomersVisible)
              .map(
                ({
                  id,
                  heroImage,
...
const jobHOC = withJob({
  work: async ({ contentful }) => {
    const [page] = await Promise.all([
      contentful.fetchSingleByContentType("customerPage")
    ]);
