...
export default class WebglScene {
	...
	createGeometries() {
		// Verify materials exist before creating geometries
		if (!this.material1 || !this.material2 || !this.material3) {
			console.error('WebglScene: Materials must be created before geometries');
			return;
		}

		// First geometry
		this.planeGeo1 = new THREE.PlaneGeometry(this.widthPlane, this.lengthPlane, this.nbSegments, this.nbSegments);
		const plane1 = new THREE.Mesh(this.planeGeo1, this.material1);
		plane1.rotation.y = Math.PI / 2;

		// Second geometry
		this.planeGeo2 = new THREE.PlaneGeometry(this.widthPlane, this.lengthPlane, this.nbSegments, this.nbSegments);
		const plane2 = new THREE.Mesh(this.planeGeo2, this.material2);
		plane2.rotation.y = Math.PI / 2;

		// Third geometry
		this.planeGeo3 = new THREE.PlaneGeometry(this.widthPlaneMini, this.lengthPlane, this.nbSegments, this.nbSegments);
		const plane3 = new THREE.Mesh(this.planeGeo3, this.material3);

		// Number of vertices (using old Geometry API with vertices array)
	...
	twistPlanes() {
		...
		for (let i = 0; i < this.nbVerticesPlane; i += 1) {
			...
			const easing = (t -= 1) * t * t + 1;

			const x1 = this.planeGeo1.vertices[i].x * easing + Math.sin(1.45 + frame / 8) * (-6 * easing);
			const x2 = this.planeGeo2.vertices[i].x * easing + Math.sin(1.45 + frame / 8) * (8 * easing);
			const x3 = this.planeGeo3.vertices[i].x * easing + Math.sin(0.88 + frame / -5) * (3 * easing);
		...
		// Mark vertices as needing update (old Geometry API)
	...
	renderScene(pixelRatio) {
		...
		// Render for the first time to don't show black canvas
	...
	loopWaveAnimation() {
		...
		for (let i = 0; i < this.nbVerticesPlane; i += 1) {
			...
			// Update z coordinate directly on vertices (old Geometry API)
