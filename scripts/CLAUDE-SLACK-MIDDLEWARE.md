...
# Slack (for sending messages — no signing secret needed, Claude for Slack handles inbound)
...
# Google Drive (supports inline JSON for deployed envs, or file path for local dev)
...
This project does NOT need a custom Slack app with event subscriptions, slash commands,
or interactivity. The conversational interface is handled by **Claude for Slack**
(Anthropic's native Slack integration).

This middleware only needs a **bot token** (`xoxb-...`) for sending outbound messages
(draft notifications, audit reports) via `chat.postMessage`. Required bot token scopes:
- `chat:write` — post messages to channels
- `chat:write.public` — post to channels the bot hasn't joined

Claude for Slack is configured separately to call this middleware's HTTP endpoints
via MCP tools or custom instructions.
