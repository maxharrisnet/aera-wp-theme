...
// Get featured or card image - use card-appropriate size for better performance
$image_url = '';
if (!empty($resource_card_image) && is_array($resource_card_image)) {
  $att = $resource_card_image['ID'] ?? $resource_card_image['id'] ?? null;
  if ($att) {
    $image_url = wp_get_attachment_image_url($att, 'webinar_card_image') ?: ($resource_card_image['url'] ?? '');
  } else {
  ...
  if ($thumbnail_id) {
    $image_url = wp_get_attachment_image_url($thumbnail_id, 'webinar_card_image') ?: $image_url;
  }
}
      ...
      <div class="newsItem__figure">
        <a href="<?php echo esc_url($link); ?>" class="newsItem__image newsItem__imageBorder" target="_blank">
          ...
          if (! empty($image_att)) {
            ...
            echo '<img class="newsItem__img newsItem__bgImage" src="' . esc_url($image_url) . '" alt="' . esc_attr($title) . '" loading="lazy" decoding="async" />';
          }
          ?>
        </a>
      </div>
